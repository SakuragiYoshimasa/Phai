Shader "Hidden/Kernel"
{
    Properties
    {
       // _PositionTex ("-", 2D) = ""{}
       // _VelocityTex ("-", 2D) = ""{}
       _OriginPostionBuffer ("-", 2D) = ""{}
       _TargetPostionBuffer ("-", 2D) = ""{}
       _CongruentPointsIndexBuffer ("-", 2D) = ""{}
       _Metamorphosity ("-", Range(0, 1.0)) = 0
       _VertexCount("-", Float) = 0
    ...
    sampler2D _OriginPostionBuffer;
    sampler2D _TargetPostionBuffer;
    sampler2D _CongruentPointsIndexBuffer;
    float4 _OriginPostionBuffer_TexelSize;
    float4 _TargetPostionBuffer_TexelSize;
    float4 _CongruentPointsIndexBuffer_TexelSize;

   	float _Metamorphosity;
   	float _VertexCount;
...
//    }
    ...
    float4 frag_update_position(v2f_img i) : SV_Target {
   		...
   		float4 origP = tex2D(_OriginPostionBuffer, i.uv);
   		float4 congP = tex2D(_CongruentPointsIndexBuffer, i.uv);

   		//float2 conguv = float2(0.5,0);

   		//if(congP.x != 0){
   		//	conguv.y =  congP.x / _VertexCount;
   		//}

   		//float4 targP = tex2D(_TargetPostionBuffer, conguv);
   		float4 targP = tex2D(_TargetPostionBuffer, i.uv);

		return (1.0 - _Metamorphosity) * origP + _Metamorphosity * targP;
    }
